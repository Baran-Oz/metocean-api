...
class Nora3Wave(MetProduct):
    ...
    def _get_near_coord(self, url: str, lon: float, lat: float):
        with xr.open_dataset(url) as ds:
            rlon, rlat = aux_funcs.find_nearest_rot_coord(
                ds.longitude, ds.latitude, lon, lat
            )
...
class Nora3WaveSub(Nora3Wave):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/nora3_subset_wave/wave_tser/"
            + date.strftime("%Y%m")
            + "_NORA3wave_sub_time_unlimited.nc"
        )
...
class NORA3WindSub(MetProduct):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/nora3_subset_atmos/wind_hourly_v2/arome3kmwind_1hr_"
            + date.strftime("%Y%m")
            + ".nc"
        )
    ...
    def _get_near_coord(self, url: str, lon: float, lat: float):
        with xr.open_dataset(url) as ds:
            x, y = aux_funcs.find_nearest_cart_coord(
                ds.longitude, ds.latitude, lon, lat
            )
...
class NORA3WindWaveCombined(MetProduct):
    ...
    def import_data(
        self, ts: TimeSeries, save_csv=True, save_nc=False, use_cache=False
    ):
        product = ts.product
        ...
        wave_files, wave_lon_near, wave_lat_near = wave.download_temporary_files(
            ts, use_cache
        )
        ...
        wind_files, wind_lon_near, wind_lat_near = wind.download_temporary_files(
            ts, use_cache
        )
        ...
        with (
            xr.open_mfdataset(wave_files) as wave_values,
            xr.open_mfdataset(wind_files) as wind_values,
        ):
            ...
            same_coords = (
                wave_lon_near == wind_lon_near and wave_lat_near == wind_lat_near
            )
            if not same_coords:
                print(
                ...
                wind_values = wind_values.drop_vars(
...
class NORACWave(MetProduct):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/norac_wave/field/ww3."
            + date.strftime("%Y%m")
            + ".nc"
        )
    ...
    def _get_near_coord(self, url: str, lon: float, lat: float):
        with xr.open_dataset(url) as ds:
            node_id = aux_funcs.distance_2points(
...
class NORA3AtmSub(MetProduct):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/nora3_subset_atmos/atm_hourly/arome3km_1hr_"
            + date.strftime("%Y%m")
            + ".nc"
        )
...
class NORA3Atm3hrSub(MetProduct):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/nora3_subset_atmos/atm_3hourly/arome3km_3hr_"
            + date.strftime("%Y%m")
            + ".nc"
        )
...
class NORA3StormSurge(MetProduct):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/stormrisk/zeta_nora3era5_N4_"
            + date.strftime("%Y")
            + ".nc"
        )
    ...
    def _get_near_coord(self, url: str, lon: float, lat: float):
        with xr.open_dataset(url) as ds:
            eta_rho, xi_rho = aux_funcs.find_nearest_rho_coord(
                ds.lon_rho, ds.lat_rho, lon, lat
            )
    ...
    def import_data(
        self, ts: TimeSeries, save_csv=False, save_nc=False, use_cache=False
    ):
        """
...
class Norkyst800(MetProduct):
    ...
    def _get_url_info(self, date: str):
        if date >= pd.Timestamp("2016-09-14 00:00:00") and date <= pd.Timestamp(
            "2019-02-26 00:00:00"
        ):
            return (
                "https://thredds.met.no/thredds/dodsC/sea/norkyst800mv0_1h/NorKyst-800m_ZDEPTHS_his.an."
                + date.strftime("%Y%m%d%H")
                + ".nc"
            )
        elif date > pd.Timestamp("2019-02-26 00:00:00"):
            return (
                "https://thredds.met.no/thredds/dodsC/fou-hi/norkyst800m-1h/NorKyst-800m_ZDEPTHS_his.an."
                + date.strftime("%Y%m%d%H")
                + ".nc"
            )
        else:
            raise ValueError(
                f"Unhandled date {str(date)} for product {self.name}. Data only valid from 2016-09-14 onwards."
            )
    ...
    def import_data(
        ...
        for i in range(len(dates)):
            ...
            if (
                i == 0
                or dates[i].strftime("%Y-%m-%d %H:%M:%S") == "2019-02-27 00:00:00"
            ):
                selection, lon_near, lat_near = self._get_near_coord(
                    url, ts.lon, ts.lat
                )
            ...
            if use_cache and os.path.exists(tempfiles[i]):
                ...
                try:
                    ...
                    if (i > 10) and not any_sucessful_download:
                        raise ValueError(
                            "Unable to access any files. This typically means the server is down."
                        )
                    continue
...
class NORA3WaveSpectrum(MetProduct):
    ...
    def _get_near_coord(self, url: str, lon: float, lat: float):
        with xr.open_dataset(url) as ds:
            station = aux_funcs.distance_2points(
                ds.latitude.values, ds.longitude.values, lat, lon
            ).argmin()
    ...
    def import_data(
        ...
        if save_csv:
            ...
            csv_data = pd.DataFrame(
                csv_data, columns=["time", "frequency", "direction", "value"]
            )
...
class NORACWaveSpectrum(MetProduct):
    ...
    def _get_url_info(self, date: str):
        return (
            "https://thredds.met.no/thredds/dodsC/norac_wave/spec/ww3_spec."
            + date.strftime("%Y%m")
            + ".nc"
        )
...
class NORA3fp(MetProduct):
    ...
    def get_dates(self, start_date, end_date):
        ...
        end_date = pd.to_datetime(end_date) + pd.Timedelta(
    ...
    def _alter_temporary_dataset_if_needed(self, dataset: xr.Dataset):
        ...
        rename_dict = {
            "height2": "height",
            "height3": "height_clouds",
            "pressure0": "pressure_level",
        }
        ...
        if "x_wind_z" in dataset and "y_wind_z" in dataset:
            dataset["wind_speed"] = np.sqrt(
                dataset["x_wind_z"] ** 2 + dataset["y_wind_z"] ** 2
            )
            dataset["wind_from_direction"] = (
                270 - np.arctan2(dataset["y_wind_z"], dataset["x_wind_z"]) * 180 / np.pi
            ) % 360
        ...
        for var_name, height in variables_to_expand:
            if var_name in dataset:
                expanded_vars[var_name] = dataset[var_name].expand_dims(
                    {"standard_height": [height]}
                )

        # Concatenate and merge variables if they exist
        if (
            "air_temperature_0m" in expanded_vars
            and "air_temperature_2m" in expanded_vars
        ):
            air_temperature = xr.concat(
                [
                    expanded_vars["air_temperature_0m"],
                    expanded_vars["air_temperature_2m"],
                ],
                dim="standard_height",
            )
        ...
        for var_name in [
            ...
            if var_name in expanded_vars:
                dataset = xr.merge(
                    [
                        dataset,
                        expanded_vars[var_name].rename(
                            var_name.replace("_2m", "").replace("_10m", "")
                        ),
                    ]
                )
    ...
    def _process_fluxes(self, ds):
        ...
        rename_dict = {
            key: working_dict[key]["_rename"]
            for key in working_dict.keys()
            if key in fluxes and working_dict[key]["_rename"] is not None
        }
    ...
    def download_temporary_files(
        ...
        tempfiles = aux_funcs.get_tempfiles(
            self.name, lon, lat, dates + pd.Timedelta(hours=4)
        )
        selection = None
        lon_near = None
        lat_near = None
        ...
        selection, lon_near, lat_near = self._get_near_coord(
            self._get_url_info(dates[0], 3), lon, lat
        )
        tqdm.write(
        ...
        header = "|{:^20}|{:^25}|{:^20}|{:^25}|".format(
            "Nb forecast", "Nb file to download", "Time per forecast", "Estimated time"
        )
        ...
        for i, forecast in enumerate(dates):
            ...
            for lead_time in range(3, 10):
                ...
                file = Path(
                    f"cache/nora_{lon_near:.3f}_{lat_near:.3f}" + url.split("/")[-1]
                )
            ...
            tqdm.write(
                f"Processing forecast data {forecast.strftime('%Y-%m-%d %H:00')}"
            )
...
class NORA3OffshoreWind(MetProduct):
    ...
    def calc_L(
        self,
        downward_northward_momentum_flux,
        downward_eastward_momentum_flux,
        specific_humidity,
        temperature,
        pressure,
        sensible_flux,
        ...
        downward_northward_momentum_flux = (
            downward_northward_momentum_flux.metpy.quantify()
        )
        downward_eastward_momentum_flux = (
            downward_eastward_momentum_flux.metpy.quantify()
        )
        ...
        tau = np.sqrt(
            downward_northward_momentum_flux**2 + downward_eastward_momentum_flux**2
        )

        # Calculate wtheta_s
        wtheta_s = (
            sensible_flux / (density * cp) * (1 + 0.6078 * specific_humidity)
        ) + (0.6078 * theta * (vap_latent_flux / (density * Lv)))
        ...
        L, friction_velocity = (
            L.metpy.convert_units("m").metpy.dequantify(),
            friction_velocity.metpy.convert_units("m/s").metpy.dequantify(),
        )

        attrs = {key: value for key, value in nora_attrs.items() if key not in "units"}

        attrs["long_name"] = (
            "Atmospheric Stability Obukhov Length Calculated Using Stulls 2009 Formula"
        )
    ...
    def import_data(
        ...
        while retry_count < max_retries:
            try:
                ...
                ts.product = fpc.name
                fpc_files, fpc_lon_near, fpc_lat_near = fpc.download_temporary_files(
                    ts, use_cache
                )
                ...
                ts.product = atm.name
                atm_files, atm_lon_near, atm_lat_near = atm.download_temporary_files(
                    ts, use_cache
                )
                ...
                distance = self._haversine(
                    fpc_lon_near, fpc_lat_near, atm_lon_near, atm_lat_near
                )
                print(
                    f"The distance between 'NORA3_fpc' grid point ({fpc_lat_near:.4f},{fpc_lon_near:.4f}) and 'NORA3_atm3hr_sub' grid point ({atm_lat_near:.4f},{atm_lon_near:.4f}) is {distance:.3f}km"
                )
                ...
                with (
                    xr.open_mfdataset(
                        fpc_files, parallel=False, engine="netcdf4"
                    ) as ds_fpc,
                    xr.open_mfdataset(
                        atm_files, parallel=False, engine="netcdf4"
                    ) as ds_atm,
                    aux_funcs.Spinner(),
                ):
                    ds_fpc = ds_fpc.load()
                    ...
                    (
                        ds_fpc["atmosphere_obukhov_length"],
                        ds_fpc["magnitude_friction_velocity_in_air"],
                    ) = self.calc_L(
                        ...
                        ds_fpc.specific_humidity.sel(
                            standard_height=2
                        ).metpy.convert_to_base_units(),
                        ds_fpc.air_temperature.sel(
                            standard_height=2
                        ).metpy.convert_to_base_units(),
                    ...
                    ds_fpc["wind_speed"] = (
                        mpcalc.wind_speed(ds_fpc.x_wind_z, ds_fpc.y_wind_z)
                        .metpy.convert_units("m/s")
                        .metpy.dequantify()
                    )
                    ds_fpc["wind_from_direction"] = (
                        mpcalc.wind_direction(ds_fpc.x_wind_z, ds_fpc.y_wind_z)
                        .metpy.convert_units("degree")
                        .metpy.dequantify()
                    )

                    # Copy attributes from x_wind_z to wind_speed and wind_from_direction, preserving MetPy attributes
                    wind_speed_attrs = {
                        **ds_fpc.x_wind_z.attrs,
                        **ds_fpc["wind_speed"].attrs,
                    }
                    wind_from_direction_attrs = {
                        **ds_fpc.x_wind_z.attrs,
                        **ds_fpc["wind_from_direction"].attrs,
                    }
                    ...
                    ws10 = mpcalc.wind_speed(
                        ds_fpc.x_wind.sel(standard_height=10),
                        ds_fpc.y_wind.sel(standard_height=10),
                    ).metpy.dequantify()
                    wd10 = mpcalc.wind_direction(
                        ds_fpc.x_wind.sel(standard_height=10),
                        ds_fpc.y_wind.sel(standard_height=10),
                    ).metpy.dequantify()
                    ...
                    relative_humidity_2m = ds_fpc.relative_humidity.sel(
                        standard_height=2
                    )
                    specific_humidity_2m = ds_fpc.specific_humidity.sel(
                        standard_height=2
                    )

                    ds_fpc["air_temperature_2m"] = air_temperature_2m
                    ds_fpc["air_temperature_2m"].attrs["long_name"] = (
                        "Air temperature at 2 meters"
                    )
                    ds_fpc["air_temperature_2m"].attrs["standard_name"] = (
                        "air_temperature"
                    )

                    ds_fpc["relative_humidity_2m"] = relative_humidity_2m
                    ds_fpc["relative_humidity_2m"].attrs["long_name"] = (
                        "Relative humidity at 2 meters"
                    )
                    ds_fpc["relative_humidity_2m"].attrs["standard_name"] = (
                        "relative_humidity"
                    )

                    ds_fpc["specific_humidity_2m"] = specific_humidity_2m
                    ds_fpc["specific_humidity_2m"].attrs["long_name"] = (
                        "Specific humidity at 2 meters"
                    )
                    ds_fpc["specific_humidity_2m"].attrs["standard_name"] = (
                        "specific_humidity"
                    )
                    ...
                    ds_fpc_to_drop = [
                        var
                        for var in ds_fpc.variables.keys()
                        if var not in ds_fpc_variables
                    ]
                    ...
                    ds_atm_tke = (
                        ds_atm.tke.rename({"height": "height_tke"})
                        .resample(time="1h")
                        .asfreq()
                        .drop_vars(["x", "y"])
                        .reindex(time=ds_fpc.time, fill_value=np.nan)
                    )
                    ds_fpc["specific_turbulent_kinetic_energy_of_air"] = ds_atm_tke
                    ds_fpc["specific_turbulent_kinetic_energy_of_air"].attrs[
                        "standard_name"
                    ...
                    new_attrs = {
                        ...
                        "min_time": pd.Timestamp(ds_fpc.time.values[0]).strftime(
                            "%Y-%m-%d %H:%M:%SZ"
                        ),
                        "max_time": pd.Timestamp(ds_fpc.time.values[-1]).strftime(
                            "%Y-%m-%d %H:%M:%SZ"
                        ),
                    ...
                    if save_nc:
                        ...
                        aux_funcs.save_to_netcdf(
                            ds_fpc, ts.datafile.replace(".csv", ".nc")
                        )
